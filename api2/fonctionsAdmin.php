...
function getEvenements($idEvenement,$dbh)
{
    ...
    A.idAppareil,
    A.titreAppareil
    from evenement E inner join appareil A on A.idAppareil=E.idAppareil ";
    if($idEvenement!="ALL") 
    {
	...
	foreach ($lignes as $colonne)
	{
        ...
        $data[$i]['titre2Evenement']=$colonne->titre2Evenement;
...
{    
    $requete="SELECT I.idImage,I.nomImage, I.dateCreationImage, I.dateReceptionImage, I.typeImage, E.domaineEvenement,E.idEvenement from image I inner join evenement E on E.idEvenement=I.idEvenement where E.idEvenement=".$idEvenement;
...
{
    $titreEvenement="Titre Défaut";
    $titre2Evenement="Sous titre Défaut";
    $actifEvenement=0;
    $idAppareil=1;
    $dateEvenement=date('Y-m-d');

    $reqInsert = $dbh->prepare("INSERT INTO evenement (titreEvenement,titre2Evenement,actifEvenement,idAppareil, dateEvenement) VALUES (?,?,?,?,?)");
    $reqInsert->bindParam(1,$titreEvenement);
    $reqInsert->bindParam(2,$titre2Evenement);
    $reqInsert->bindParam(3,$actifEvenement);
    $reqInsert->bindParam(4,$idAppareil);
...
function uploadTemplate($target_dir, $nomInputFile, $type)
{
    $base64=NULL;
    $return=array();
    $ext = pathinfo($_FILES[$nomInputFile]["name"], PATHINFO_EXTENSION);
    ...
    if ($_FILES[$nomInputFile]["size"] > 1000000)
    {
        $erreur.="Votre fichier est trop lourd et doit peser moins de 1mo";
        $uploadOk = 0;
    }
