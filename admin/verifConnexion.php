	...
	if ((isset($_POST['emailAdministrateur']) && !empty($_POST['emailAdministrateur'])) && (isset($_POST['passAdministrateur']) && !empty($_POST['passAdministrateur']))) 
	{
        ...
        foreach ($lignes as $colonne)
        {
            if ($colonne->nbre == 1) 
            {
                ...
                echo $value = $colonne->idAdministrateur.'---'.hash('sha512', $colonne->emailAdministrateur.'---'.$colonne->passAdministrateur."---_Admin");
                setcookie('authSelfyFun', $value, time() + (7 * 24 * 3600) , "/", $domaineCookie);
                echo "Cookie: ".$_COOKIE['authSelfyFun'];
